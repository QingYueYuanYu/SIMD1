...
int main()
{
	int *R = (int*) aligned_malloc(32*n*n,32);
	int *E = (int*) aligned_malloc(32*n*n,32);
	srand(time(0));
	time_t dsec=0;
	long dnsec=0;
	...
	for(int i=0;i<m;i++)
	{
		...
		{
			IsNullHelpE = _mm256_load_si256((__m256i_u*)&E[i*n+j]);
		...
		while(IsNull)
		{
			...
			for(int j=0;j+8<n;j+=8)
			{
				IsNull2HelpR = _mm256_load_si256((__m256i_u*)&R[lp(&E[i*n])*n+j]);
			...
			if(IsNull2)
			{
				...
				for(int j=0;j+8<n;j+=8)
				{
					vtE = _mm256_load_si256((__m256i_u*)&E[i*n+j]);
					vtR = _mm256_load_si256((__m256i_u*)&R[FirstNum*n+j]);
					vtE = _mm256_xor_si256(vtE,vtR);
					_mm256_store_si256((__m256i_u*)&E[i*n+j],vtE);
				}
			}
			else
			{
				...
				{
					vtE = _mm256_load_si256((__m256i_u*)&E[i*n+j]);
					_mm256_store_si256((__m256i_u*)&R[FirstNum*n+j],vtE);
					_mm256_store_si256((__m256i_u*)&E[i*n+j],vtE2);
				}
				break;
			}
