...
int main()
{
	int *R = (int*) aligned_malloc(16*n*n,16);
	int *E = (int*) aligned_malloc(16*n*n,16);
	srand(time(0));
	time_t dsec=0;
	long dnsec=0;
	...
	for(int i=0;i<m;i++)
	{
		...
		{
			IsNullHelpE = _mm_load_si128((__m128i_u*)&E[i*n+j]);
		...
		while(IsNull)
		{
			...
			for(int j=0;j+4<n;j+=4)
			{
				IsNull2HelpR = _mm_load_si128((__m128i_u*)&R[lp(&E[i*n])*n+j]);
			...
			if(IsNull2)
			{
				...
				for(int j=0;j+4<n;j+=4)
				{
					vtE = _mm_load_si128((__m128i_u*)&E[i*n+j]);
					vtR = _mm_load_si128((__m128i_u*)&R[FirstNum*n+j]);
					vtE = _mm_xor_si128(vtE,vtR);
					_mm_store_si128((__m128i_u*)&E[i*n+j],vtE);
				}
			}
			else
			{
				...
				{
					vtE = _mm_load_si128((__m128i_u*)&E[i*n+j]);
					_mm_store_si128((__m128i_u*)&R[FirstNum*n+j],vtE);
					_mm_store_si128((__m128i_u*)&E[i*n+j],vtE2);
				}
				break;
			}
